...
@property (nonatomic, strong)MxCountDownLabel *countLabel;
  
@end


@implementation MxRadarView

+ (MxRadarView *)scanRadarViewWithRadius:(CGFloat)radius angle:(int)angle radarLineNum:(int)radarLineNum hollowRadius:(CGFloat)hollowRadius {
...
+ (MxRadarView *)diffuseRadarViewWithStartRadius:(CGFloat)startRadius endRadius:(CGFloat)endRadius circleColor:(UIColor *)circleColor {
...
//        [self drawRadarLine];
        ...
        for (int i = 0; i < _angle; i++) {
            UIColor * color = [self colorWithCurrentAngleProportion:i*1.5/_angle];
